...
export function getState() { //获取state
    ...
    caizhanRequest.post("/live/state", {
        ...
        if (res.data.code == 1) {
            Store.commit("saveStateInfo", res.data.state);
        } else {
...
export const commitInfo = params => caizhanRequest.post("/running/activity", params); //礼品数据登记

export const checkActivity = params => caizhanRequest.post("/running/checkActivity", params); //查看活动是否过期

export const getGiftList = params => caizhanRequest.post("/running/getAll", params); //奖品列表

export const getStationInfo = params => caizhanRequest.post("/stationinfo/getByStationnum", updateParams(params)); //根据站点号查询

export const getStationInfoByName = params => caizhanRequest.post("/stationinfo/getByStationnumName", updateParams(params)); //根据名字和站点号查询

export const getGift = params => caizhanRequest.post("/gift/generate", params); //获取礼品（深圳利是封）

export const getStationRemain = params => caizhanRequest.post("/gift/listgift", params); // 站点库存（深圳利是封）

export const postPaper = params => caizhanRequest.post("gift/sendnewspaper", params); // 领取报纸（深圳利是封）

export const getGiftRecord = params => caizhanRequest.post("/gift/listorder", updateParams(params)); //领取信息
